...
- **Cross-device sync (optional)** — connect your own Google Drive to sync events, contacts, categories, circles, and settings across devices. Optionally also push your events to a dedicated **Orbit** calendar in Google Calendar, so they show up alongside your other calendars, or pull events from your other Google calendars into Orbit (read-only). See [Cross-device sync](#cross-device-sync-optional) below.
...
3. **APIs & Services → OAuth consent screen** → User type **External** → app name "Orbit" → leave **Publishing status: Testing** (this skips Google's app-review process, which isn't needed for a personal app) → add your own Gmail address under **Test users** → add the scopes `https://www.googleapis.com/auth/drive.file`, `https://www.googleapis.com/auth/calendar.events`, and `https://www.googleapis.com/auth/calendar.readonly`.
...
### Also importing from Google Calendar

A third toggle, **Import Google Calendar**, does the reverse: it pulls events from all your *other* Google calendars (everything except the "Orbit" calendar above) into Orbit, so they show up on your Orbit week view alongside your native events. Imports cover a rolling window — one month back to six months ahead — and refresh automatically when you open or foreground Orbit.

Imported events are **read-only**: they show with a small 🔒 on their pill, can't be dragged, resized, edited, or deleted in Orbit, and are never pushed back to any of your Google calendars (including the Orbit calendar). To change one, edit it directly in Google Calendar — Orbit will pick up the change on its next refresh. Turning the toggle off stops future imports but leaves already-imported events in place until the next refresh prunes them. This also reuses the Drive sign-in, with the extra `calendar.readonly` scope from step 3 above — re-consent if you connected before that scope existed.

## Data & privacy

All events and contacts are stored in your browser's `localStorage` on your device. Nothing is sent to a server unless you explicitly connect Google Drive sync (see above), in which case your data is written only to a single file in *your own* Drive — never to any server we operate, because there is none. If you also turn on Google Calendar sync, your event titles, times, and notes are written to the "Orbit" calendar in *your own* Google Calendar — same trust boundary, still nothing touches a server we control. If you turn on Google Calendar import, the reverse happens read-only: event titles, times, and notes are read from your other Google calendars into Orbit's local storage, and nothing is written back. Clearing your browser data (or using "Clear all data" in Settings) removes everything locally; disconnecting sync in Settings stops syncing but leaves the Drive file, the Orbit calendar, and your local data intact.
